  ...
  return (
    <div className="space-y-4">
      <Card>
        ...
        <div className="flex items-center gap-4 mb-4 text-xs text-gov-textSec">
          ...
          <div className="h-52 -ml-2 mt-4">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 5, right: 12, bottom: 0, left: 0 }}>
                ...
                <XAxis dataKey="tick" tick={{ fontSize: 10, fill: '#4b5563' }} />
                <YAxis yAxisId="left" tick={{ fontSize: 10, fill: '#4b5563' }} width={30} />
                <YAxis
                  yAxisId="right"
                  orientation="right"
                  domain={[0, 100]}
        ...
        <Card>
          ...
          <div className="space-y-2">
            {swarmAlerts.map((alert) => (
              <div
                key={alert.trapId}
                className="flex items-center justify-between px-3 py-2 rounded bg-gov-bg border border-gov-border"
              >
                <div className="min-w-0">
                  <p className="text-sm font-bold text-gov-navy truncate">{alert.label}</p>
                  <p className="text-xs text-gov-textSec">{alert.pestLabel}</p>
                </div>
