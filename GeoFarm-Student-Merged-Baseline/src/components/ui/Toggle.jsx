    ...
    >
      ...
      {(label || description) && (
        <span className="flex flex-col">
          {label && <span className="text-sm font-bold text-gov-navy">{label}</span>}
          {description && <span className="text-xs text-gov-textSec">{description}</span>}
        </span>
      )}
    </label>
  );
    ...
    <div className="inline-flex rounded-lg border border-gray-300 bg-gray-100 p-0.5">
      {options.map((opt) => (
        <button
          ...
          className={`rounded-md font-bold transition-colors ${padClass} ${
            value === opt.value
              ? 'bg-white text-gov-blue shadow-sm'
              : 'text-gray-500 hover:text-gov-navy'
          }`}
        >
          {opt.label}
        </button>
