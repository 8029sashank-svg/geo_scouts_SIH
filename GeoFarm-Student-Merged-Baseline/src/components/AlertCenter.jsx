      ...
      <Card>
        ...
        {dispatchedAlerts.length === 0 ? (
          <div className="py-10 flex flex-col items-center gap-2 text-gov-textSec">
            ...
            <p className="text-xs text-gray-500">{t('noDispatchedHint')}</p>
          </div>
        ) : (
          <div className="space-y-2">
            {dispatchedAlerts.map((alert) => (
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-3 rounded bg-gov-bg hover:bg-gray-100 border border-gov-border transition-colors text-left"
              >
                <div className="w-9 h-9 rounded bg-white border border-gov-border flex items-center justify-center shrink-0">
                  <Send size={16} className="text-gov-blue" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className={`text-sm font-bold text-gov-navy truncate ${isMarathi ? 'font-devanagari' : ''}`}>
                    {alert.title}
                  </p>
      ...
      >
        {previewAlert && (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 text-xs text-gov-textSec">
            ...
            <div className="relative">
              <pre
                className={`bg-gray-50 border border-gray-200 rounded p-4 text-xs overflow-x-auto max-h-72 overflow-y-auto whitespace-pre-wrap ${
                  channel === 'ivr' && previewLang === 'mr' ? 'font-devanagari' : ''
                } ${channel === 'ivr' ? 'text-gray-800 leading-relaxed' : 'text-blue-800'}`}
              >
                {payloadText}
              </pre>
              <button
                onClick={handleCopy}
                className="absolute top-2 right-2 p-1.5 rounded bg-gray-200 hover:bg-gray-300 text-gray-700 transition-colors"
