...
import { Map as MapIcon, MapPin, ArrowRight, Layers, Box } from 'lucide-react';
import { Card, CardHeader } from '../../components/ui/Card.jsx';
import { useScout } from '../ScoutContext.jsx';
import { CLUSTER_VISITS, MISSION_STATUS } from '../mockData.js';
...
export default function FieldMap({ onOpenMission, onNavigate }) {
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="h-[26rem]">
            <MapContainer center={CENTER} zoom={9} style={{ height: '100%', width: '100%' }}>
              <TileLayer
                ...
                maxZoom={16}
              />
              <CircleMarker center={SCOUT_LOCATION} radius={8} pathOptions={{ color: '#fff', weight: 2, fillColor: '#174A7E', fillOpacity: 1 }}>
                <Popup>
                  <div className="text-xs font-semibold">📍 My Location (demo)</div>
                </Popup>
              </CircleMarker>
              {liveVisit && (
                <>
                ...
                >
                  <Popup>
                    <div className="min-w-[180px] text-xs">
                      ...
                      <p className="mb-1">Current risk: <span className="font-bold">{m.priority}</span></p>
                      <p className="mb-2">Last report: {m.lastObservation}</p>
              ...
              {trapList.map((t) => (
                <CM key={t.id} center={jitter(CENTER, t.id)} radius={6} pathOptions={{ color: '#fff', weight: 2, fillColor: '#0284c7', fillOpacity: 1 }}>
                  <Popup><div className="text-xs"><p className="font-bold text-gov-navy mb-1">🟦 {t.id}</p><p>{t.location}</p></div></Popup>
                </CM>
              ))}
            </MapContainer>
          </div>
      ...
      {mode === '2d' && (
        <Card>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <Legend color="#174A7E" label="My Location (demo)" />
          </div>
        </Card>
      )}
      ...
      <Card>
        <CardHeader title="Assigned Fields" />
        <div className="space-y-2">
          {missions.map((m) => (
            <button key={m.id} onClick={() => onOpenMission(m.id)} className="w-full flex items-center justify-between text-left bg-gov-bg border border-gov-border rounded-lg px-3 py-2.5 hover:border-gov-blue">
              <div className="min-w-0">
                ...
                <p className="text-xs text-gov-textSec">{m.crop} • {m.lastObservation}</p>
              </div>
...
function jitter(center, seed) {
  ...
  const dx = ((hash % 100) / 100 - 0.5) * 0.6;
  const dy = (((hash >> 4) % 100) / 100 - 0.5) * 0.6;
